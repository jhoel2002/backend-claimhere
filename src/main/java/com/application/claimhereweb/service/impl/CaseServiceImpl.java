...
public class CaseServiceImpl implements ICaseService {
     ...
     * 
     * @Transactional
     * public ResponseCaseDTO saveCase(SaveCaseDTO dto, Long id_customer, Long
     * id_area) {
     * // logger.info("Registrando caso con título: {}", dto.getTitle());
     * 
     * // String areaName =
     * Optional.ofNullable(areaRepository.findNameById(id_area))
     * // .orElseThrow(() -> {
     * // return new ResourceNotFoundException("Área no encontrada");
     * // });
     * 
     * String customerName =
     * Optional.ofNullable(customerRepository.findCustomerUserNameById(id_customer))
     * .orElseThrow(() -> {
     ...
     * legalCase.setCustomer(new Customer() {{ setId(id_customer); setUser(new
     * User() {{ setName(customerName); }}); }});
     * legalCase.setCaseStatus(CaseStatus.Nuevo);
     * 
